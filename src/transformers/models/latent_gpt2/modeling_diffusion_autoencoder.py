...
class LanguageDiffusionDecoder(LanguageDecoderBase):
    ...
    def pre_process_inputs(
        ...
        Args:
            ...
            inputs_latents (`torch.FloatTensor` of shape `(batch_size, seq_len, latent_dim)`, *optional*):
    ...
    def post_process_outputs(
        ...
        Args:
            outputs ([`CausalLMAutoencoderOutputWithCrossAttentions`]):
                ...
                - `last_hidden_state`: Full hidden states from the final layer
        ...
        Returns:
            [`BaseAutoencoderOutputWithPastAndCrossAttentions`]:
                ...
                - `last_hidden_state`: Full reshaped hidden states
                - `past_key_values`: Unchanged cache states
                - `hidden_states`: Tuple of reshaped hidden states per layer
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, seq_len)`, *optional*):
                Masked token sequence. Positions containing `mask_token_id` will be predicted
                by the model. If None, a fully masked sequence is generated internally.
            inputs_latents (`torch.FloatTensor` of shape `(batch_size, num_latents, latent_dim)`, *optional*):
                Latent representations from the encoder to condition the decoding.
            inputs_embeds (`torch.FloatTensor` of shape `(batch_size, seq_len, hidden_size)`, *optional*):
                Pre-computed embeddings (alternative to `inputs_latents`).
            return_segment (`bool`, *optional*, defaults to `True`):
                Whether to reshape outputs back to batch dimensions.
...
class LanguageDiffusionAutoencoder(GPT2PreTrainedModel, GenerationMixin):
    ...
    Example:
        ...
        latents = outputs.latents  # (batch_size, num_segments, latent_dim)
        ```
    """
        ...
        **kwargs,
        ...
        Returns:
            [`CausalLMAutoencoderOutputWithCrossAttentions`] or `tuple`:
                ...
                - `latents`: Compressed latent vectors `(batch, num_segments, latent_dim)`
                - `last_tail_hidden_state`: Final hidden state per segment
        ...
        Example:
            ...
            latents = encoder_output.latents  # (batch, seq_len // window_size, latent_dim)
            ```
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
    ...
    def decode(
        ...
        **kwargs,
        ...
        Note:
            This method currently references `encoder_output` which is not defined
            in the method scope. It should be called after `encode()` or the
            `inputs_embeds` parameter should be used to provide the encoder output.

        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, seq_len)`, *optional*):
                Masked token sequence for diffusion decoding. Positions with
                `mask_token_id` will be predicted by the model.
            inputs_latents (`torch.FloatTensor` of shape `(batch_size, num_segments, latent_dim)`, *optional*):
        ...
        Returns:
            [`CausalLMAutoencoderOutputWithCrossAttentions`] or `tuple`:
                ...
                - `logits`: Token prediction logits `(batch, num_segments * window_size, vocab_size)`
                - `last_window_hidden_state`: Hidden states for decoded positions
                - `hidden_states`: All layer outputs (if requested)
                - `attentions`: Attention weights (if requested)
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, seq_len)`, *optional*):
                Input token IDs to encode. The sequence is compressed by a factor
                of `window_size` into latent representations.
            inputs_embeds (`torch.FloatTensor`, *optional*):
        ...
        Returns:
            [`CausalLMAutoencoderOutputWithCrossAttentions`] or `tuple`:
                If `return_encoder_decoder_res=False` (default):
                    ...
                    - `latents`: Encoded latent vectors `(batch, num_segments, hidden_size)`
                    - `last_hidden_state`: Decoder final layer hidden states
                    - `hidden_states`: All layer hidden states (if requested)
                    - `attentions`: Attention weights (if requested)
